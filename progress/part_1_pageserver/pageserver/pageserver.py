...
def respond(sock):
    ...
    parts = request.split()
    #create path for file location
    options = get_options()
    DOCROOT = options.DOCROOT
    if len(parts) > 1:
        location = parts[1][1:]
    path = os.path.join(DOCROOT, location)
    log.info("Requested File Path: {}\n".format(path))
    if("/~" in  parts[1] or "//" in  parts[1] or "/.." in  parts[1]):
        log.info("Forbidden Path: {}\n".format(path))
        transmit(STATUS_FORBIDDEN, sock)
        transmit("Forbidden Path: {}\n".format(path), sock)
    else:
        check = os.path.exists(path);
        if(check == False):
            log.info("Requested File Path Not Found.")
            transmit(STATUS_NOT_FOUND, sock)
            transmit("Requested File Not Found: {}\n".format(path), sock)
        elif(check==True):
